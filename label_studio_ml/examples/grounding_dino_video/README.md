...
2. Run docker compose
    ...
    The backend now supports batch processing to better utilize your GPU resources. By default, it uses a batch size of 8, but you can adjust this based on your GPU memory:
    ...
    # Or via CLI argument
    python cli.py --batch-size 16 --tasks 1,2,3
    ```
    
    For a 48GB GPU, you can typically use batch sizes of 16-32 depending on your video resolution.
    
    ### Mixed Precision (FP16) for Faster Inference
    
    The backend automatically enables mixed precision (FP16) when running on CUDA devices, which can speed up inference by 2-3x.
    
    ### Saving Annotated Frames for Debugging
    
    To debug tracking issues, you can save annotated frames with bounding boxes, labels, and tracking IDs:
    
    ```bash
    # Save frames to default directory (./output_frames)
    python cli.py --save-frames --tasks 1,2,3
    
    # Save frames to custom directory
    python cli.py --save-frames --output-dir /path/to/output --tasks 1,2,3
    ```
    
    Each frame will be saved as `frame_XXXXXX.jpg` with visualizations including:
    - Bounding boxes with different colors per track
    - Track IDs (e.g., "#12")
    ...
    ```bash
    # Batch size for parallel frame processing (default: 8)
    export GROUNDING_DINO_BATCH_SIZE=16
    
    # Base resolution for input images (default: 800)
    export GROUNDING_DINO_BASE_SIZE=800
    
    # Maximum resolution (default: 1333)
    export GROUNDING_DINO_MAX_SIZE=1333
    
    # Progress logging interval (default: 25 frames)
    export GROUNDING_DINO_PROGRESS_EVERY=50
    ```
    ...
    This will:
    1. Process tasks 1, 2, and 3
    2. Use batch size of 16 for better GPU utilization
    3. Save annotated frames to `./debug_output/task_1/`, `./debug_output/task_2/`, etc.
    4. Upload predictions to Label Studio
